...
class TraceStack:
  ...
  def pop(self) -> None:
    master_trace = self.stack.pop()

  def __repr__(self) -> str:
    return  'Trace stack\n{}\n{}\n{}'.format(
      map('  {}\n'.format, self.stack[::-1]),
      self.dynamic, self.executor)
...
def reset_trace_state() -> bool:
  ...
  if (trace_state.substack != [Sublevel(0)] or
      ...
      trace_state.trace_stack.stack or
      trace_state.trace_stack.executor != base_executor):
...
def find_top_trace(xs):
  ...
  return top_master and top_master.trace_type(top_master, cur_sublevel())
